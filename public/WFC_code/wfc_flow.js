...
function setupProbGraph(folder) {
  ...
  var selected_tile;
  if(folder.includes('CITY')) {
    var strings = ['Grass','Sand','Lagoon','Ocean','Wall']
  ...
  var sumvar =  points.reduce((sum, item) => sum + item.Y(), 0);
  ...
  points.forEach((point,pointindex) =>{
    point.on('drag', function(e) {
      var notthispoint = points.filter(otherpoint => otherpoint.getName()!=point.getName())
      var adjustment = (point.Y()-oldys[pointindex])*-1/(notthispoint.length)
      var bad_adj
