...
Playwright records video for every test (`video: 'on'` in `playwright.config.ts`). On every PR push, the workflow stitches per-browser videos **side-by-side** (chromium │ firefox │ webkit) into one MP4 + animated WebP and **upserts a single PR comment** (marker `<!-- playwright-video-report -->`) with the WebP embedded inline. The `mobile-chrome` project runs its own mobile-only spec (`tests/e2e/mobile.spec.ts`); its videos upload as raw artifacts but are not stitched into the combined comment (different test set → different stream length would truncate the hstack).
