...
<Figure src="" caption="On the left, the red and green colours are added to each other, so that an impression of the densities of both species can be found. On the right, only the species with the greater density is rendered, in a first-past-the-post system." number='2'/>

<Note> <p>Be careful!</p> </Note>
